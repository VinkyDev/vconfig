...
export default function ConfigList({
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys: React.Key[]) => {
      // 处理全选/取消全选
      if (
        selectedRowKeys.length === configs.length &&
        ...
        const newKey =
          selectedRowKeys.find(
            (key) => !selectedConfigs.includes(key as string)
  ...
  if (configs.length === 0) {
    return (
      <Empty description="暂无配置项" style={{ padding: "60px 0" }}>
        <Text type="secondary">点击上方新建配置按钮添加第一个配置</Text>
      </Empty>
    );
  }
